...
!function () {
    window.HE = {
        config: {
            ...
            autoHeight: !1,
            uploadPhoto: true,
            uploadPhotoHandler: "http://192.168.1.168:5000/uploadfile",
            uploadPhotoSize: 0,
            uploadPhotoType: "gif,png,jpg,jpeg",
            uploadPhotoTypeError: "只能上传图片文件",
            uploadPhotoSizeError: "上传的图片文件超过允许的大小",
            maxUploadPhotoWidth = 450,
            maxUploadPhotoQuality = 0.9,
            uploadParam: {},
        ...
        handyEditor: function (id, options) {
            ...
            ediv.className = "HandyEditor_editor", ediv.setAttribute("contenteditable", !0), this.config.autoHeight ? (ediv.style.minHeight = this.config.height, ediv.style.height = "auto", ediv.style.overflowY = "hidden") : (ediv.style.height = this.config.height, ediv.style.overflow = "auto"), this.define.obj = ediv, hediv.appendChild(mkdiv);
            ...
            if (HE.define.mobj.blur(), document.getElementById("HandyEditor_newdiv")) HE.deldiv(); else {
                ...
                if (n.className = "HandyEditor_newdiv", n.id = "HandyEditor_newdiv", "fontsize" == e) for (var d = [14, 16, 18, 24, 32, 48], t = '<div class="HandyEditor_newdiv_down" id="HandyEditor_newdiv_down_1" style="font-size:10px;" draggable="true">' + HE.define.lang.fontsize + "</div>", o = 0; o < d.length; o++) t += '<div class="HandyEditor_newdiv_gap"></div><div class="HandyEditor_newdiv_down" id="HandyEditor_newdiv_down_' + (o + 2) + '" style="font-size:' + d[o] + 'px;" draggable="true">' + HE.define.lang.fontsize + "</div>"; else if ("fontname" == e) {
                    ...
                    if (1 == HE.config.uploadPhoto) {
                        ...
                        // t += '<div><input id="HandyEditor_newdiv_down_upload_input" type="file" accept="image/*" name="file" style="display:none;"><span class="HandyEditor_menu_item" id="HandyEditor_newdiv_down_upload" style="display: inline-block;margin-left: 10px;">' + HE.define.lang.uploadimage + '</span><span id="HandyEditor_newdiv_down_upload_percent" style="display: inline-block;margin-left: 10px;"></span></div>'
                    }
                ...
                if (i.offsetLeft < n.offsetWidth / 2 && (H = 5), HE.define.mobj.offsetWidth - i.offsetLeft - i.offsetWidth < n.offsetWidth / 2 && (H = HE.define.mobj.offsetWidth - n.offsetWidth - 5), n.style.left = H + "px", n.onclick = function () {
                    ...
                    if (document.getElementById("HandyEditor_newdiv_down_imageok").onclick = HE.imageokEx, 1 == HE.config.uploadPhoto) {
                        ...
                        u.onchange = function () {
                            ...
                            for (var t in d) {
                                if (HE.trim(d[t].toLowerCase()) == n.toLowerCase()) {
                                    e = 0;
                                    break
                                }
                            }
        ...
        sarea: function (e) {
            ...
            if(!HE.define.select){
                HE.define.select = i.getRangeAt(0)
            }
            // console.log('sarea---',HE.define.select, e)
            e = e || !1,
                window.getSelection ? (i = window.getSelection(),
                    e ? (i.removeAllRanges(),
                        i.addRange(HE.define.select)) :
                        HE.define.select = i.getRangeAt(0)) :
                    e ? (document.body.createTextRange().select(),
                        document.selection.empty(),
                        document.selection.addRange(HE.define.select)) :
                        HE.define.select = document.selection.createRange(),
        ...
        changeImg: function (img) {
            ...
            if (img) {
                ...
                var base64 = HE.blobToDataURL(img, function (base64Url) {
                    document.getElementById("HandyEditor_newdiv_down_image").value = base64Url;
        ...
        getOrientation: function (arrayBuffer) {
            ...
            let orientation, exifIDCode, tiffOffset, firstIFDOffset, littleEndian, endianness, app1Start, ifdStart, offset;
            ...
            if (app1Start) {
                ...
                if (HE.getStringFromCharCode(dataView, exifIDCode, 4) === 'Exif') {
        ...
        compressImage: function(file, success, error) {
            let maxUploadPhotoWidth = HE.config.maxUploadPhotoWidth, maxUploadPhotoQuality = HE.config.maxUploadPhotoQuality
            // 图片小于1M不压缩
            if (maxUploadPhotoWidth <= 0) {
                return success(file);
            }
            ...
            reader.onload = e => {
                ...
                const ori = HE.getOrientation(HE.base64ToArrayBuffer(src))
                // const ori = 6;

                const img = new Image();
                img.src = src;
                img.onload = e => {
                    const maxWidth = maxUploadPhotoWidth;
                    let imgWidth = img.width;
                    let imgHeight = img.height;
                    const quality = maxUploadPhotoQuality;  // 默认图片质量为0.92
                    if (imgWidth > maxWidth){
                        imgHeight = parseInt(maxWidth * imgHeight / imgWidth)
                        imgWidth = maxWidth
                    }
                    ...
                    // 返回base64转blob的值
                    // alert(`原图${(src.length/1024).toFixed(2)}kb ` + `新图${(base64.length/1024).toFixed(2)}kb`);
                    // 去掉url的头，并转换为byte
        ...
        uploadFile: function (e) {
            ...
            HE.compressImage(e, function (e) {
                ...
                o.onreadystatechange = function() {
                    ...
                    if (4 == e.readyState){
                        if (200 == e.status) {
                            ...
                            // if (/^(http:\/\/|https:\/\/|\/)?(([\w\-]+\.)*[\w\-]+(\:\d+)?\/)?([\w\-]+\/)*[\w\-]+\.[A-Za-z]{2,4}$/i.test(n)) {
                            document.getElementById("HandyEditor_newdiv_down_image").value = n;
                            document.getElementById("HandyEditor_newdiv_down_imageok").click()
                            // }else
                            //     alert(e.response)
                        } else
                            alert(HE.define.lang.uploadimageerror)
                    }
                };
                ...
                //     }
                // }, !1);
                o.open("post", HE.config.uploadPhotoHandler, false);
                o.send(n);
            }, function (err) {
                alert("压缩图片失败")
            })
